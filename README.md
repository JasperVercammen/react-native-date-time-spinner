...
| `stabilizeWidth`             | `boolean`                                                   | `true`                     | Reserve each column's width for its widest label up front, so the column doesn't grow (shoving neighbours aside) when a longer label scrolls in on mount. No-op for uniform-width numeric columns. |
